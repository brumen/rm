...
class MarketService:
    """ Gathering information about the market. Runs on a ticker. For a certain period of time,
          the data is collected, and then published.
    """
    ...
    def __init__( self
        ...
        :param server_port_topic: tuple of (name of the kafka server, port nb, topic to read from)
        """
        ...
        self.__mkt_listener = KafkaConsumer(bootstrap_servers=f'{server_name}:{port}')
        ...
        self.__mkt_producer = KafkaProducer(bootstrap_servers=f'{server_name}:{port}')
...
class AOMarketService(MarketService):
    ...
    def encode_mkt(self) -> Tuple[UUID, Dict[str, float]]:
        ...
        return latest_market_id, { f"{flight_id}|{flight_date.strftime('%Y%m%d')}": flight_price
                                   for (flight_id, flight_date), flight_price in latest_market.items() }

    @staticmethod
    def _decode_to_tuple(enc_str_date : str) -> Union[None, Tuple[str, datetime.date]]:
    ...
    @staticmethod
    def decode_mkt(encoded_mkt : Tuple[UUID, Dict[str, float]]):
        """
