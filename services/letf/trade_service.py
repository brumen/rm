...
class LETFTradeProducer(BaseProducer):
    ...
    def _value_1(self, trade_nb: int):
        ...
        letf_position = {
            "LETF": {
                ...
                "amount": amount,
                "beta": self._beta,
                "stock_value": 30.0,  # TODO: WHAT IS THIS SUPPOSED TO BE???
            }
        }
        return letf_position

    def _value_perp(self, trade_nb: int):
        perp_stock = random.choice(self._stocks)
        amount = np.random.rand() * 100  # rand() is between 0-1.
        perp_position = {
            "Perp": {
                "trade_id": str(trade_nb),  # str(uuid4()),
                "underlying": perp_stock,
                "amount": amount,
                "initial_spot": 30.0,  # TODO: WHAT IS THIS SUPPOSED TO BE???
            }
        }
    ...
    def publish_few_values(self, nb_letfs: int, trade_nb_start: int):
        for letf_idx in range(nb_letfs):
            self.publish_value(self._value_1(trade_nb_start + letf_idx * 5))

    def _value_to_publish(self, sleep_between_publish=11.0):
        ...
        while True:
            ...
            yield letf_position
            perp_position = self._value_perp(trade_nb + 1)
            yield perp_position
            trade_nb += 5
