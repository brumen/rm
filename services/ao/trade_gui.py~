...
def new_trade_added( trade_nb        : int
                     , pv_label      : Label
                     , flights_frame : Frame
                     , ):
...
def create_frame(flights : List[Flight], frame : Frame):
    ...
    for id_col, id_elt in enumerate(('ID', 'ORIGIN', 'DEST', 'DEP', 'ARR', 'CARRIER', 'PRICE')):
...
def add_trade():
    ...
    new_trade_button = Button( root
                               , text = 'Compute'
                               , command=lambda : new_trade_added(entry_nb.get(), new_trade_pv, flights_frame))\
...
add_trade_button = Button(root, text='Add trade', command=lambda : add_trade()).grid(row=0, column=0)
