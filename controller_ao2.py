...
from rm.flow.input_classes import FlightMemorizer
...
class ControllerAO(Controller):
    ...
    def __init__( self
                ...
                , mkt_rester      : str  = 'https://localhost:5000/mkt/get_market'
        ...
        # trade handling.
        self._trade_memo = FlightMemorizer('flight_memo'
                                           , server      = server_name
                                           , port        = port
                                           , topic_input = positions_topic
                                           , queue_list  = [self._trade_queue_curr_market, self._trade_queue_new_market ] )
    ...
    def _compute_trade_from_mkt( cls
        ...
        aof = AirOptionFlights( mkt_date, flights, ao_trade.strike)  # default constructor for AO
    ...
    def _value_trade_id(cls, mkt_date_trade_id : Tuple[datetime.date, Tuple[int, str]], db_session = None) -> Union[None, RES_TYPE]:
        ...
        return cls._value_trade((mkt_date, get_trade(trade_id, db_session), trade_direction))
    ...
    def _read_mkt_events(self):
        ...
        for msg in self.__mkt_listener:
            # TODO: FIX THIS
            # market_id should be obtained from msg
            # get the market from the rester service
            response = requests.get(api_url)
            market_id, market = response.json()
            logger.debug(f'New market event: {market_id}')
    ...
    def start( self
        ...
        self._trade_memo.run()  # start the
