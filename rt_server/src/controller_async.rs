...
impl Controller {
    ...
    async fn _value_trade(&self, trade: Trade) -> TradeValue {
        let trade_id = trade.trade_id;

        //"http://localhost:5010/pv/{trade_id}"
        let result_pricing = reqwest::get(format!("http://{}/pv/{}", self.trade_pricer, trade_id))
            .await?
            .text()
            .await?;
    ...
    async fn _value_portfolio(&mut self, trade_queue: String, nb_elts: usize) -> PortfolioType {
        ...
        for trade in trades_all {
            let trade_value = self._value_trade(trade).await;
    ...
    async fn _trade_processor_curr(&mut self, sleep_delay: Duration) {
        loop {
            ...
            if queue_size != 0 {
                ...
                let trade_values = self
                    ._value_portfolio(String::from("curr"), queue_size)
                    .await;
    ...
    async fn _trade_processor_new(&mut self, sleep_delay: Duration) {
        loop {
            ...
            if queue_size != 0 {
                ...
                let trade_values = self._value_portfolio(String::from("new"), queue_size).await;
