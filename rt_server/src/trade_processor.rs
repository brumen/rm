...
pub trait TradeMarketDiscovery<TT>
where TT:  PartialEq + std::fmt::Debug + Clone {
    ...
    TT:  PartialEq + std::fmt::Debug + Clone + BaseTrade + PriceTrade
{
    ...
    fn _process_trade(
        ...
        let trade_v = trade.value_by_metric(
            trade_id,
            self.metric(),
            CurrNewMarket::Current,  // TODO: THIS IS WRONG HERE
        );
        ...
        loop {
	        ...
	        while let Ok(trade) = trade_receiver.try_recv() {
                ...
                if !all_trades.contains(&trade) {
                    ...
                    debug!("_trade_processor_curr: Processing trade {}, dir {:?}", trade_id, trade_direction);
                    let trade_v = trade.value_by_metric  (trade_id, self.metric(), CurrNewMarket::Current);
                    debug!("_trade_processor_curr: Trade value = {:?}", trade_v);
                    match trade_direction {
...
where
    T: MarketSwitching,  // + PriceMultipleTrades<TT>,
    TT: BaseTrade + PartialEq + std::fmt::Debug + Clone + PriceTrade,
{ }


/// Pricing engine for trades for remote pricing
pub trait RiskProcessorsRemote<TT> : MarketSwitching + TradeMarketDiscovery<TT>
where
    TT:  PartialEq + std::fmt::Debug + Clone + BaseTrade + PriceTradeAsync
{
    ...
    async fn _process_trade(
        ...
        debug!("_trade_processor_curr: Processing trade {}, dir {:?}", trade_id, trade_direction);
        let trade_v = trade.value_by_metric(trade_id, self.metric(), CurrNewMarket::Current).await;
        debug!("_trade_processor_curr: Trade value = {:?}", trade_v);
        match trade_direction {
            TradeDirection::Create => curr_portfolio += trade_v,
            TradeDirection::Delete => curr_portfolio -= trade_v,
